describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let shadowRoot = await page.evaluateHandle((element) => element.shadowRoot, prodItem);
    let button = await shadowRoot.$('button');
    await button.click();
    let innerText = await page.evaluate((element) => element.innerText, button);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let shadowRoot = await page.evaluateHandle((element) => element.shadowRoot, prodItem);
      let button = await shadowRoot.$('button');
      let buttonInnerText = await page.evaluate((element) => element.innerText, button);
      ...
      let cartCountInnerText = await cartCountElement.evaluate((node) => node.innerText);
